...
DECLARE
  N NUMBER(3);
BEGIN
  SELECT COUNT(*) INTO N FROM DBA_USERS WHERE USERNAME = 'VENTAS';
  IF(N = 1) THEN
    EXECUTE IMMEDIATE 'DROP USER VENTAS CASCADE';
  END IF;
  EXECUTE IMMEDIATE 'CREATE USER VENTAS IDENTIFIED BY admin';  
END;
/
