...
CREATE OR REPLACE PROCEDURE VENTAS.ACTUALIZA_STOCK(
	P_CODPROD IN VARCHAR,
	P_CANTIDAD IN VARCHAR,
	P_COD_RESPUESTA OUT NUMBER,
  P_MENSAJE 			OUT VARCHAR2)
IS
	V_PRODUCTO VENTAS.PRODUCTO%ROWTYPE;
	V_NUEVO_STOCK VENTAS.PRODUCTO.STOCK%TYPE;
BEGIN
	...
	VENTAS.PKG_PRODUCTO.ACTUALIZAR(V_PRODUCTO.CODPROD,V_PRODUCTO.NOMBRE,V_PRODUCTO.MARCA,V_PRODUCTO.PRECIO,V_PRODUCTO.DESCUENTO,
...
CREATE OR REPLACE PROCEDURE VENTAS.REGISTRAR_VENTA(
...
IS
  ...
  V_FILAS  VENTAS.PKG_UTIL.T_ARRAY_STRING;
  V_CAMPOS VENTAS.PKG_UTIL.T_ARRAY_STRING;
...
BEGIN
	...
	-- PROCESAR CLIENTE
	V_CLIENTE := VENTAS.PKG_UTIL.SIGUIENTE_VALOR(V_CADENA_VENTA,'$');
	IF(UPPER(SUBSTR( V_CLIENTE, 1, 1)) = 'S') THEN
		V_CAMPOS := VENTAS.PKG_UTIL.SPLIT(V_CLIENTE,'|');
		VENTAS.PKG_CLIENTE.INSERTAR(V_CAMPOS(2),V_CAMPOS(3),V_CAMPOS(4),V_CAMPOS(5),V_CAMPOS(6),V_CAMPOS(7),V_COD_RESPUESTA,V_MENSAJE); 
		IF (V_COD_RESPUESTA <> 0) THEN
			RAISE_APPLICATION_ERROR(-20030, V_MENSAJE);
		END IF;	
	END IF;
	V_CADENA_VENTA := VENTAS.PKG_UTIL.RESTO(V_CADENA_VENTA,'$');
	
	-- PROCESAR VENTA
	V_VENTA := VENTAS.PKG_UTIL.SIGUIENTE_VALOR(V_CADENA_VENTA,'$');
	V_CAMPOS := VENTAS.PKG_UTIL.SPLIT(V_VENTA,'|');
	VENTAS.PKG_VENTA.INSERTAR(V_CAMPOS(1),SYSDATE,TO_NUMBER(V_CAMPOS(2)),V_CAMPOS(3),V_CAMPOS(4),V_COD_RESPUESTA,V_MENSAJE); 
	IF (V_COD_RESPUESTA <> 0) THEN
	...
	-- PROCESAR PAGO
	V_PAGO := VENTAS.PKG_UTIL.SIGUIENTE_VALOR(V_CADENA_VENTA,'$');
	V_CAMPOS := VENTAS.PKG_UTIL.SPLIT(V_PAGO,'|');
	VENTAS.PKG_PAGO.INSERTAR(SYSDATE,V_CAMPOS(1),V_CAMPOS(2),V_CAMPOS(3),V_CAMPOS(4),V_COD_RESPUESTA,V_MENSAJE); 
	IF (V_COD_RESPUESTA <> 0) THEN
	...
	-- PROCESAR DETALLES
	V_FILAS := VENTAS.PKG_UTIL.SPLIT(V_CADENA_VENTA,'$');
	FOR I IN 1 .. V_FILAS.COUNT LOOP        
		--VERIFICA Y ACTUALIZA STOCK
		V_CAMPOS := VENTAS.PKG_UTIL.SPLIT(V_FILAS(I),'|');
  	VENTAS.ACTUALIZA_STOCK(V_CAMPOS(2),V_CAMPOS(3),V_COD_RESPUESTA,V_MENSAJE);
  	IF (V_COD_RESPUESTA <> 0) THEN
			RAISE_APPLICATION_ERROR(-20033, V_MENSAJE);
		END IF;
    --AÑADE DETALLE
    VENTAS.PKG_DETALLE_VENTA.INSERTAR(V_CAMPOS(1), V_CAMPOS(2), V_CAMPOS(3), V_CAMPOS(4), V_CAMPOS(5), V_COD_RESPUESTA,V_MENSAJE );
  	IF (V_COD_RESPUESTA <> 0) THEN
...
--END;
	...
	VENTAS.REGISTRAR_VENTA(V_PRUEBA,V_COD_RPTA,V_MENSAJE);
